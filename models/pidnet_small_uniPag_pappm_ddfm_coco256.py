...
class PIDNet_S(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=19, planes=64, spp_planes=128, head_planes=128, augment=False):
        ...
        self.dfm = model_utils.DDFM(planes * 4, planes * 8)
        ...
        self.final_layer = segmenthead(planes * 8, head_planes, num_classes)
...
def PIDNet_imagenet(cfg, pretrained=True):
    model = PIDNet_S(BasicBlock, [2, 2, 3, 3], num_classes=171, planes=32, spp_planes=96, head_planes=256, augment=True)
    if pretrained:
...
if __name__ == '__main__':
    ...
    model = PIDNet_S(BasicBlock, [2, 2, 3, 3], num_classes=19, planes=32, spp_planes=96, head_planes=256, augment=False)
    model.eval()
    model.to(device)
    iterations = None
    ...
    """
    model = PIDNet_S(BasicBlock, [2, 2, 3, 3], num_classes=19, planes=32, spp_planes=96, head_planes=128, augment=True)
