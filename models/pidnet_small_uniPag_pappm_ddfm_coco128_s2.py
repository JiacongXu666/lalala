...
class PAPPM(nn.Module):
    def __init__(self, inplanes, branch_planes, outplanes, BatchNorm=nn.BatchNorm2d):
        ...
        self.scale_process = nn.Sequential(
                                    ...
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(branch_planes*3, branch_planes*3, kernel_size=3, padding=1, groups=3, bias=False),
                                    )
...
def PIDNet_imagenet(cfg, pretrained=True):
    model = PIDNet_S(BasicBlock, [2, 2, 3, 3], num_classes=171, planes=32, spp_planes=96, head_planes=128, augment=True)
    if pretrained:
...
if __name__ == '__main__':
    ...
    model = PIDNet_S(BasicBlock, [2, 2, 3, 3], num_classes=171, planes=32, spp_planes=96, head_planes=128, augment=False)
    model.eval()
    model.to(device)
    iterations = None
