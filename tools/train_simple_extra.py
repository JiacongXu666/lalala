...
#    train_sampler = get_sampler(train_dataset)
    ...
    if config.LOSS.USE_OHEM:
        ...
        sb_criterion = OhemCrossEntropy(ignore_label=config.TRAIN.IGNORE_LABEL,
                                        thres=config.LOSS.OHEMTHRES,
                                        min_kept=config.LOSS.OHEMKEEP,
                                        weight=train_dataset.class_weights)
    else:
        sem_criterion = CrossEntropy(ignore_label=config.TRAIN.IGNORE_LABEL,
                                    weight=train_dataset.class_weights)
        sb_criterion = CrossEntropy(ignore_label=config.TRAIN.IGNORE_LABEL,
                                    weight=train_dataset.class_weights)
    bd_criterion = JointBondaryLoss()
    
    model = FullModel(model, sem_criterion, bd_criterion, sb_criterion)
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if flag_rm == 1 or epoch % 5 == 0 or (epoch > end_epoch - 100):
